...
//	public void test (JButton tglbtnR) {
...
//	}
	...
	public void updateDisplay(Measurements measurements, Alerts alerts) {
		if (alerts.isESR()) 
			ESRPanel.setBackground(Color.YELLOW); //DOUBT
		else
			ESRPanel.setBackground(Color.WHITE); //Warnings are always in yellow
		if (alerts.isPDMG()) {
		...
		if (alerts.isPackage_not_delivered()) {
		...
		if (alerts.isPOS()) {
			POSPanel.setBackground(Color.RED); //alarms should be red 
		...
		if (alerts.isPWR60()) {
			PWR60Panel.setBackground(Color.RED); // alarms should be red
		...
		if (alerts.isDC()) {
			DCPanel.setBackground(Color.RED); // alarms should be in red
		...
		if (alerts.isISRZ())
			ISRZPanel.setBackground(Color.GREEN); //HAVE CHANGED FROM RED TO GREEN
		else
...
//
	...
	public void addComponents (Measurements measurements, Alerts alerts) {
